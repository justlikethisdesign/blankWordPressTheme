...
function get_transient_array($transName, $function, $lifespan) {
    ...
    $transient_name = 'BLANKDOMAINNAME_tran_'.$transName. '_' . BLANKDOMAINNAME_theme_version();
    ...
    // Nope!  We gotta make a call.
        ...
        if($out){
            ...
            //Get transient time
            $lifespan = BLANKDOMAINNAME_general_transient_lifespan($lifespan);

            set_transient( $transient_name, $out, $lifespan );
            BLANKDOMAINNAME_store_transient_in_list( $transient_name );
...
function BLANKDOMAINNAME_general_transient_lifespan($lifespan) {
...
function BLANKDOMAINNAME_clean_down_all_transients(){

    $transient_list = BLANKDOMAINNAME_get_transient_list();
 ...
 * and stores it in the Transient Array.
 *
 * This array can be used to mass clean all
 * transients
 */

function BLANKDOMAINNAME_store_transient_in_list( $transient_name ){
    ...
    update_option( 'BLANKDOMAINNAME_transient_storage', $transient_list );

}



function BLANKDOMAINNAME_get_transient_list(){

    $transient_list = get_option('BLANKDOMAINNAME_transient_storage');
