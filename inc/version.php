 ...
 * @since BLANKDOMAINNAME 1.0.0
 */
function BLANKDOMAINNAME_get_file_version_number( $name, $file ){

    $timestamp = get_file_last_mod_stamp( $file );

    if( $name && $timestamp ){

        $version_num = BLANKDOMAINNAME_set_file_version_number( $name, $timestamp );
 ...
 */
function BLANKDOMAINNAME_set_file_version_number( $name, $timestamp ){

    $version_option_name = 'BLANKDOMAINNAME' . $name . '_version_number';
    ...
    //BLANKDOMAINNAME_inc_version_number can take an empty string, or a 3 point number
    $new_version = BLANKDOMAINNAME_inc_version_number( $current_version );
 ...
 * @since BLANKDOMAINNAME 1.0.0
 */
function BLANKDOMAINNAME_inc_version_number( $version = null ){
 ...
 * @since BLANKDOMAINNAME 1.0.0
 */
function BLANKDOMAINNAME_check_modif_date($timestamp, $old_stamp){
